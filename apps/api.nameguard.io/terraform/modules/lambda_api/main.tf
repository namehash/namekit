...
resource "aws_lambda_function" "nameguard_lambda" {
  ...
  tags = merge(local.common_tags, {
    Function = "nameguard-api"
  })
}

resource "aws_lambda_provisioned_concurrency_config" "concurrency_config" {
  function_name                     = aws_lambda_function.nameguard_lambda.function_name
  provisioned_concurrent_executions = 1
  qualifier                         = aws_lambda_function.nameguard_lambda.version
}
...
resource "aws_cloudfront_distribution" "api_distribution" {
  enabled             = true
  comment             = "Distribution for nameguard API ${var.env}"
