...
async def call_subgraph(network_name: NetworkName, query: str, variables: dict) -> dict:
    try:
        ...
        raise ENSSubgraphUnavailable(
            f"Communication error with subgraph occurred: {ex}"
        )
...
async def namehash_to_name_lookup(
    network_name: NetworkName, namehash_hexstr: str
) -> str:
...
def build_multi_label_query_querying_labelhashes(
    labelhashes: list[str]
) -> tuple[str, dict]:
    """
    Builds a query that resolves all labelhashes in a name.
...
async def resolve_labelhashes_querying_labelhashes(
    network_name: NetworkName, labelhashes: list[str]
) -> dict[str, str]:
    """
...
async def resolve_all_labelhashes_in_name_querying_labelhashes(
    network_name: NetworkName, name: str
) -> str:
    logger.debug(f"Trying to resolve full name: {name}")
