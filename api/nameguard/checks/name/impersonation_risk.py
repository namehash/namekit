...
MESSAGE_SKIP_UNK = (
    "Contains unknown labels and cannot be checked for impersonation risk"
)
    ...
    if passed:
        ...
        for label in labels:
            for grapheme in label.graphemes:
                if (
                    grapheme.type == "emoji"
                    and grapheme.confusables_canonical != grapheme.value
                ):
                    CHANGED_EMOJI = True
                    break
        if CHANGED_EMOJI:
            ...
            message = MESSAGE_FAIL.format(
                ".".join(label.beautiful_canonical_label for label in labels)
            )
    else:
