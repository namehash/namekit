...
async def inspect_name_post(request: InspectNameRequest) -> NameGuardReport:
    logger.debug(
        f"{json.dumps({'endpoint': Endpoints.INSPECT_NAME, 'method': 'POST', 'network_name': request.network_name, 'name': request.name})}"
    )
    nameguard.context.endpoint_name.set(Endpoints.INSPECT_NAME)
...
async def bulk_inspect_names(
    request: BulkInspectNamesRequest
) -> BulkNameGuardBulkReport:
    logger.debug(
        f"{json.dumps({'endpoint': Endpoints.BULK_INSPECT_NAMES, 'method': 'POST', 'network_name': request.network_name, 'names': request.names})}"
    )
...
async def inspect_labelhash_post(request: InspectLabelhashRequest) -> NameGuardReport:
    ...
    return await ng.inspect_namehash(
        network_name=request.network_name, namehash=namehash
    )
...
async def secure_primary_name_get(
    address: str, network_name: NetworkName
) -> SecurePrimaryNameResult:
    logger.debug(
...
async def fake_eth_name_check_get(
    ...
    return await ng.fake_eth_name_check(
        network_name=network_name, contract_address=contract_address, token_id=token_id
    )
