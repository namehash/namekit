...
def test_inspect_name_post_latin_all_pass(test_client):
    ...
    # labels keys
    assert (
        set(label_res.keys()) == {'label', 'labelhash', 'normalization', 'highest_risk', 'rating', 'risk_count',
                                  'checks',
                                  'graphemes'}
    ...
    assert all(
        set(grapheme_res.keys()) == {'normalization', 'grapheme', 'grapheme_name', 'grapheme_type', 'grapheme_script',
                                     'highest_risk', 'rating', 'risk_count', 'grapheme_description', 'subtitle',
                                     'title'}
...
def test_bulk_inspect_name_post(test_client):
    ...
    assert all(
        [set(r.keys()) == {'name', 'namehash', 'normalization', 'highest_risk', 'rating', 'risk_count',
                           'beautiful_name', 'subtitle', 'title'} for r in res_json['results']]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('mainnet', '0XEe6c4522aab0003e8d14cd40a6af439055fd2577951148c14b6cea9a53475835', 200, 'vitalik.eth'),
        # uppercase hex
        ('mainnet', '107841754600925073349285697024366035838042340511934381588201623605284409137205', 200,
         'vitalik.eth'),
...
def test_inspect_namehash_post(test_client, namehash: str, expected_status_code: int, expected_name: str):
    network_name = 'mainnet'
    response = test_client.post('/inspect-namehash',
...
@pytest.mark.parametrize(
    ...
    [
        ('0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045', 'unlikely', 'normalized', 'vitalik.eth', 'vitalik.eth',
         'vitalik.eth', False, 'vitalik.eth'),
        ('0x8Ae0e6dd8eACe27045d9e017C8Cf6dAa9D08C776', 'potential', 'normalized', 'vitalìk.eth', 'vitalìk.eth',
         'vitalik.eth', True, 'vitalìk.eth'),
        ('0x8B7863d67e1083EE1becbDD277cbBFf1c1CCB631', 'unlikely', 'normalized', '٧٣٧.eth', '٧٣٧.eth', '٧٣٧.eth', False,
         '٧٣٧.eth'),
        ('0xFD9eE68000Dc92aa6c67F8f6EB5d9d1a24086fAd', 'unlikely', 'normalized', 'exampleprimary.cb.id',
         'exampleprimary.cb.id',
         'exampleprimary.cb.id', False, 'exampleprimary.cb.id'),
        (
                '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96046', None, 'no_primary_name', None, 'Unnamed d8da', None,
                False, None),
        ('0xfA9A134f997b3d48e122d043E12d04E909b11073', None, 'unnormalized', None, 'Unnamed fa9a', None, False,
         '888‍‍.eth'),
        ('0x76fd9b1B2d8F2cd9Ba06c925506627883F97B97C', None, 'unnormalized', None, 'Unnamed 76fd', None, False,
         '‍‍❤‍‍.eth'),
        ('0xf537a27F31d7A014c5b8008a0069c61f827fA7A1', None, 'unnormalized', None, 'Unnamed f537', None, False,
         '٠٠۱.eth'),  # normalizable
        ('0x0ebDfD75d33c05025074fd7845848D44966AB367', None, 'unnormalized', None, 'Unnamed 0ebd', None, False,
         '۸۸۷۵۴۲.eth'),  # normalizable
        ('0xaf738F6C83d7D2C46723b727Ce794F9c79Cc47E6', None, 'unnormalized', None, 'Unnamed af73', None, False,
         '୨୨୨୨୨.eth'),  # canonical can be '99999.eth'
        ('0xb281405429C3bc91e52707a21754cDaCeCbB035E', None, 'unnormalized', None, 'Unnamed b281', None, False,
         '┣▇▇▇═─.eth'),
        ('0x0d756ee0e8C250f88f5e0eDd7C723dc3A0BF75cF', None, 'unnormalized', None, 'Unnamed 0d75', None, False,
         'сбер.eth'),  # canonical can be 'c6ep.eth'
        ('0x7Da3CdE891a76416ec9D1c3354B8EfE550Bd4e20', None, 'unnormalized', None, 'Unnamed 7da3', 'vitalik.eth', True,
         'vitȧlik.eth'),
        ('0xC9f598BC5BB554B6A15A96D19954B041C9FDbF14', None, 'unnormalized', None, 'Unnamed c9f5', 'vitalik.eth', True,
         'vıtalik.eth'),
        ('0x7c7160A23b32402ad24ED5a617b8a83f434642d4', 'unlikely', 'normalized', 'vincξnt.eth', 'vincΞnt.eth',
         'vincξnt.eth', False, 'vincξnt.eth'),
        ('0x744Ec0A91D420c257aE3eE471B79B1A6a0312E36', None, 'unnormalized', None, 'Unnamed 744e', None, False,
         'hello<world>!.eth'),  # attempt code injection
...
def test_primary_name(test_client, address, impersonation_status, primary_name_status, primary_name,
    ...
    if primary_name_status != 'no_primary_name':
        ...
        assert any(check['check'] == 'impersonation_risk' and check['status'] == 'warn' for check in
                   res_json['nameguard_result']['checks']) == impersonation_risk
...
def test_primary_name_get_emoji(test_client):
    ...
    assert res_json['nameguard_result']['highest_risk'][
               'message'] == 'Emojis used in this name may be visually confused with other similar emojis'
...
@pytest.mark.parametrize(
    ...
    [
        ('0x495f947276749ce646f68ac8c248420045cb7b5e',
         ...
         FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),  # fake nick.eth
        ('0x2cc8342d7c8bff5a213eb2cde39de9a59b3461a7', '45104', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        ('0x495f947276749ce646f68ac8c248420045cb7b5e',
        ...
        ('0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d', '3972', FakeEthNameCheckStatus.NON_IMPERSONATED_ETH_NAME),
        ('0xbd3531da5cf5857e7cfaa92426877b022e612cf8', '2028', FakeEthNameCheckStatus.NON_IMPERSONATED_ETH_NAME),
        ('0xbd3531da5cf5857e7cfaa92426877b022e612cf9', '2028', FakeEthNameCheckStatus.UNKNOWN_NFT),  # NOT_A_CONTRACT
        ('0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '47192814855232171824620094590612668126513223473283784600320596656451859494352',
         FakeEthNameCheckStatus.AUTHENTIC_ETH_NAME),  # brantly.eth
        ('0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '0X68562Fc74af4dcfac633a803c2f57c2b826827b47f797b6ab4e468dc8607b5d0',
         FakeEthNameCheckStatus.AUTHENTIC_ETH_NAME),  # brantly.eth uppercase hex
        ('0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '0x68562fc74af4dcfac633a803c2f57c2b826827b47f797b6ab4e468dc8607b5d0',
         FakeEthNameCheckStatus.AUTHENTIC_ETH_NAME),  # brantly.eth
        ('0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '0xaf498306bb191650e8614d574b3687c104bc1cd7e07c522954326752c6882770', FakeEthNameCheckStatus.UNKNOWN_ETH_NAME),
        # unknown but registered #TODO 'title': '[0xaf49...2770].eth'
        ('0X57F1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '47192814855232171824620094590612668126513223473283784600320596656451859494352',
         FakeEthNameCheckStatus.AUTHENTIC_ETH_NAME),  # brantly.eth uppercase hex
        ('0X57F1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '0x37bf77d30d63cbf9ddad6b3c161522c53dcdcd8177b6177c83835c5ea69a7f8f', FakeEthNameCheckStatus.UNKNOWN_NFT),
        # random ENS name
        ('0xfe4f558a0fee0657bfa044792f5545f5a8f4ecb1', '1', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        ...
        ('0x47dD5F6335FfEcBE77E982d8a449263d1e501301', '79', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        # https://x2y2.io/eth/0x47dD5F6335FfEcBE77E982d8a449263d1e501301/79
        ('0x2Cc8342d7c8BFf5A213eb2cdE39DE9a59b3461A7', '8107', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        # https://x2y2.io/eth/0x2Cc8342d7c8BFf5A213eb2cdE39DE9a59b3461A7/8107
        ('0xd8B287885cAb9E377de8F61f000Ff9B3F50e2F4d', '4', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        # https://foundation.app/@rogerhaus/vitalik/4
        ('0xd8B287885cAb9E377de8F61f000Ff9B3F50e2F4d', '0x4', FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME),
        # token id as hex
        ('0x6B175474E89094C44Da98b954EedeAC495271d0F', '1', FakeEthNameCheckStatus.UNKNOWN_NFT),
        # NO_SUPPORTED_NFT_STANDARD
        ('0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', '1', FakeEthNameCheckStatus.UNKNOWN_NFT),
        # NO_SUPPORTED_NFT_STANDARD
        ('0xA478c2975Ab1Ea89e8196811F51A7B7Ade33eB11', '1', FakeEthNameCheckStatus.UNKNOWN_NFT),
        # NO_SUPPORTED_NFT_STANDARD
    ...
    [
        ('0x495f947276749ce646f68ac8c248420045cb7b5e',
         ...
         {'contractMetadata.name': 'OpenSea Shared Storefront',
          'contractMetadata.openSea.collectionName': 'OS Shared '
                                                     'Storefront '
                                                     'Collection',
        ...
        ('0x2cc8342d7c8bff5a213eb2cde39de9a59b3461a7',
         ...
         {'contractMetadata.name': 'ETHRegistrarLinageeWrapper',
          'contractMetadata.openSea.collectionName': 'Linagee '
                                                     'Name '
                                                     'Registrar '
                                                     '(LNR)',
        ...
        ('0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d',
         ...
         {'contractMetadata.name': 'BoredApeYachtClub',
          'contractMetadata.openSea.collectionName': 'Bored Ape '
                                                     'Yacht '
                                                     'Club',
          'title': ''},
        ...
        ('0xbd3531da5cf5857e7cfaa92426877b022e612cf8',
         ...
         {'contractMetadata.name': 'PudgyPenguins',
          'contractMetadata.openSea.collectionName': 'Pudgy '
                                                     'Penguins',
        ...
        ('0x57f1887a8bf19b14fc0df6fd9b2acc9af147ea85',
         '47192814855232171824620094590612668126513223473283784600320596656451859494352',
         'brantly.eth',
         {'contractMetadata.openSea.collectionName': 'ENS: '
                                                     'Ethereum '
                                                     'Name '
                                                     'Service',
...
def test_fake_eth_name_check_fields(test_client, contract_address, token_id, title, fields, fake,
    ...
    response = test_client.post('/fake-eth-name-check-fields', json=json_req)
    assert response.status_code == 200
    res_json = response.json()
    assert res_json['status'] == fake
    if res_json['status'] in (FakeEthNameCheckStatus.INVALID_ETH_NAME,
