...
def test_inspect_name_get_unnormalized(test_client):
    ...
    assert (
        res_json["namehash"]
        == "0x3f97a45d7aa341b55b749b276f699278634ce4e5afa3e0753a8fe5be5d1355c2"
    )
    ...
    assert res_json["labels"]
    assert all(
        [label["labelhash"] for label in res_json["labels"]]
    )  # labelhash for unnormalized
    assert all(
        [label["graphemes"] for label in res_json["labels"]]
    )  # graphemes for unnormalized
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "iam/alice?.eth",
            False,
            marks=pytest.mark.xfail(reason="not urlencoded"),
        ),
...
def test_inspect_name_get_special_characters(
    test_client, encoded_input_name: str, decoded_name: str, do_quote: bool
):
    if do_quote:
...
def test_inspect_name_post_latin_all_pass(test_client):
    ...
    response = test_client.post(
        "/inspect-name", json={"name": name, "network_name": "mainnet"}
    )
    assert response.status_code == 200
    res_json = response.json()
    pprint(res_json)
    ...
    for check in res_json["checks"]:
        ...
        assert check["status"] in (
            "pass",
            "info",
        )  # TODO remove info when fuses check implemented
    ...
    for grapheme_res, expected_grapheme in zip(
        res_json["labels"][0]["graphemes"], list("vitalik")
    ):
...
def test_bulk_inspect_name_post(test_client):
    ...
    response = test_client.post(
        "/bulk-inspect-names", json={"names": names, "network_name": "mainnet"}
    )
    assert response.status_code == 200
    res_json = response.json()
    pprint(res_json)
...
def test_inspect_namehash_get_unknown_status(
    test_client, namehash: str, normalization: str, expected_name: str
):
    network_name = "mainnet"
    response = test_client.get(f"/inspect-namehash/{network_name}/{namehash}")
    assert response.status_code == 200
...
def test_inspect_namehash_post(
    test_client, namehash: str, expected_status_code: int, expected_name: str
):
    network_name = "mainnet"
    response = test_client.post(
        "/inspect-namehash", json={"namehash": namehash, "network_name": network_name}
    )
    assert response.status_code == 200
    res_json = response.json()
    pprint(res_json)
...
def test_inspect_labelhash_get_empty(test_client):
    labelhash = labelhash_from_label(
        "dcjq92834vhh8teru5903wu9hawtpyhuoidfj09q2yh987euitvhgs"
    )
    response = test_client.get(
        f"/inspect-labelhash/mainnet/{labelhash}"
    )  # empty parent = ''
    assert response.status_code == 404
    assert (
        response.json()["detail"]
        == "Provided namehash could not be found in ENS Subgraph."
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "0x97bf1a722288db8edaf0f6687b4431ac96feb13d3b55a7b11e9c6bc33f938bef",
            None,
            404,
            None,
        ),
...
def test_inspect_labelhash_post(
    test_client, labelhash, parent, expected_status_code, expected_name
):
    network_name = "mainnet"
...
def test_inspect_grapheme(test_client):
    ...
    assert res_json["canonical_grapheme"] in [
        confusable["grapheme"] for confusable in res_json["confusables"]
    ]
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason="wrong collection name returned by Alchemy?"
            ),
        ),
        ...
        pytest.param(
            ...
            FakeEthNameCheckStatus.IMPERSONATED_ETH_NAME,
            marks=pytest.mark.xfail(
                reason="why not works? it is on x2y2 but delisted on opensea"
            ),
        ),
...
def test_fake_eth_name_check(
    test_client, contract_address, token_id, fake, monkeypatch
):
    network_name = "mainnet"

    response = test_client.get(
        f"/fake-eth-name-check/{network_name}/{contract_address}/{token_id}"
    )
    assert response.status_code == 200
    res_json = response.json()
...
def test_fake_eth_name_check_fields_missing_title(
    test_client, contract_address, token_id, title, fields, monkeypatch
):
    network_name = "mainnet"
...
def test_fake_eth_name_check_error(test_client, monkeypatch):
    ...
    async def return_mock_response(*args, **kwargs):
        return httpx.Response(
            200,
            content=open(
                f"{pytest.TESTS_DATA_PATH}/get_nft_metadata__ERROR__{contract_address}__{token_id}.json"
            ).read(),
        )
