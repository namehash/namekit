...
async def test_lookup(name, namehash, expected):
    ...
    assert (
        await resolve_all_labelhashes_in_name_querying_labelhashes(network_name, name)
        == expected
    )
...
async def test_long_lookup():
    network_name = "mainnet"
    name = (
        "[2af8fae91ee5ef94f17f2c2f23532cc2d1ccaee78cae52efed0df04bc2463b13]." * 250
        + "eth"
    )
    assert (
        await resolve_all_labelhashes_in_name_querying_labelhashes(network_name, name)
    ...
    name = (
        "[2af8fae91ee5ef94f17f2c2f23532cc2d1ccaee78cae52efed0df04bc2463b13]." * 500
        + "eth"
    )
    assert (
        await resolve_all_labelhashes_in_name_querying_labelhashes(network_name, name)
...
async def test_lookup2():
    ...
    result = await resolve_labelhashes_querying_labelhashes(
        network_name, labelhashes.keys()
    )
    print(result)
