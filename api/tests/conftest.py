    ...
    if use_monkeypatch:
        ...
        async def mock_get_nft_metadata(contract_address: str, token_id: str) -> dict:
            ...
            try:
                return json.load(
                    open(
                        f"{TESTS_DATA_PATH}/get_nft_metadata__{contract_address}__{token_id}.json"
                    )
                )
                ...
                json.dump(
                    result,
                    open(
                        f"data/NEW-get_nft_metadata__{contract_address}__{token_id}.json",
                        "w",
                    ),
                    indent=2,
                    ensure_ascii=False,
        ...
        monkeypatch.setattr(
            "nameguard.nameguard.get_nft_metadata", mock_get_nft_metadata
        )
        ...
        def mock_name(self, address):
            try:
                ...
                json.dump(
                    result,
                    open(f"data/NEW-name__{address}.json", "w"),
                    indent=2,
                    ensure_ascii=False,
                )
                raise
        ...
        async def mock_call_subgraph(network_name, query: str, variables: dict):
            h = hashlib.md5(
                json.dumps((network_name, query, variables), sort_keys=True).encode(
                    "utf-8"
                )
            ).hexdigest()
            mapping = {
            ...
            try:
                ...
                json.dump(
                    result,
                    open(f"data/NEW-call_subgraph__{h}.json", "w"),
                    indent=2,
                    ensure_ascii=False,
                )
                raise
