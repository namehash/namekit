...
async def test_basic_red(nameguard: NameGuard):
    ...
    for check in result.checks:
        if check.check is Check.NORMALIZED:
            ...
            Check.PUNYCODE_COMPATIBLE_LABEL,
            Check.PUNYCODE_COMPATIBLE_NAME,
        ):
...
async def test_bulk(nameguard: NameGuard):
    result = await nameguard.bulk_inspect_names(
        "mainnet", ["nick.eth", "nićk.eth", "ni_ck.eth"]
    )
...
@pytest.mark.parametrize(
    "label_length", [3, 10, 62, 63, 64, 200, 240, 252, 253, 254, 255, 256, 300]
)
@pytest.mark.asyncio
async def test_bulk_simple_name(nameguard: NameGuard, label_length):
    ...
    result_bulk = await nameguard.inspect_name(
        "mainnet", ("a" * label_length) + ".eth", bulk_mode=True
    )
...
async def test_unknown_label(nameguard: NameGuard):
    ...
    assert (
        r.labels[0].label
        == "[56d7ba27aed5cd36fc16684baeb86f73d6d0c60b6501487725bcfc9056378075]"
    )
...
async def test_canonicals(nameguard: NameGuard):
    ...
    assert (
        r.canonical_name
        == "nick.[56d7ba27aed5cd36fc16684baeb86f73d6d0c60b6501487725bcfc9056378075].eth"
    )
    assert r.labels[0].canonical_label == "nick"
    assert (
        r.labels[1].canonical_label
        == "[56d7ba27aed5cd36fc16684baeb86f73d6d0c60b6501487725bcfc9056378075]"
    )
...
def test_generic_check_result_operators():
    ...
    assert GenericCheckResult(
        ...
        check=Check.FONT_SUPPORT, status=CheckStatus.WARN, _name_message="", _title=""
    )
    assert GenericCheckResult(
        check=Check.NORMALIZED, status=CheckStatus.INFO, _name_message="", _title=""
    ) < GenericCheckResult(
        check=Check.FONT_SUPPORT, status=CheckStatus.PASS, _name_message="", _title=""
    )
    assert GenericCheckResult(
        check=Check.NORMALIZED, status=CheckStatus.WARN, _name_message="", _title=""
    ) <= GenericCheckResult(
        check=Check.FONT_SUPPORT, status=CheckStatus.ALERT, _name_message="", _title=""
    )
    assert GenericCheckResult(
        check=Check.NORMALIZED, status=CheckStatus.PASS, _name_message="", _title=""
    ) > GenericCheckResult(
        check=Check.CONFUSABLES, status=CheckStatus.PASS, _name_message="", _title=""
    )
    assert GenericCheckResult(
...
async def test_dynamic_check_order(nameguard: NameGuard):
    ...
    r = await nameguard.secure_primary_name(
        "0xc9f598bc5bb554b6a15a96d19954b041c9fdbf14", "mainnet"
    )
    ...
    r = await nameguard.secure_primary_name(
        "0xd8da6bf26964af9d7eed9e03e53415d37aa96045", "mainnet"
    )
