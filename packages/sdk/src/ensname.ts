...
import { encodeLabelhash, isEncodedLabelhash, labelhash, normalizeEncodedLabelhash } from "./hashutils";
...
export function labelsEqual(labels1: string[], labels2: string[]): boolean {
    return labels1.length === labels2.length &&
 ...
 */
export function getDecentralizationStatus(name: ENSName): DecentralizationStatus {
...
export function getRegistrationPotential(name: ENSName): RegistrationPotential {
    ...
    switch (getNamespaceRoot(name)) {
        case "ens":
            if (ethRegistrarControllerLength(name.labels[name.labels.length - 2]) < MIN_ETH_REGISTRABLE_LABEL_LENGTH) {
